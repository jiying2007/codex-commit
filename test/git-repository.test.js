...
const repo = createGitRepository({
  runProcess: async (command, args, options) => {
    ...
    if (args[0] === 'log') return {
      stdout: 'fix(core): repair race\0feat(core): add guard\0chore(core): update metadata\0',
      stderr: ''
    };
    if (args[0] === 'ls-tree') return { stdout: `100644 blob deadbeef\t.codex-commit.json\0`, stderr: '' };
    if (args[0] === 'show') return { stdout: '{"language":"en","styleHistoryLimit":12}', stderr: '' };
...
(async () => {
  ...
  const expectedFingerprint = crypto.createHash('sha256').update(indexBytes).digest('hex');
  assert.strictEqual(await repo.getIndexFingerprint('/repo'), expectedFingerprint);
  assert.deepStrictEqual(await repo.getRepositorySnapshot('/repo'), {
    headOid: head,
    indexFingerprint: expectedFingerprint
  });
  assert(repo.repositorySnapshotsEqual(
    { headOid: head, indexFingerprint: expectedFingerprint },
    { headOid: head, indexFingerprint: expectedFingerprint }
  ));
  assert(!repo.repositorySnapshotsEqual(
    { headOid: head, indexFingerprint: expectedFingerprint },
    { headOid: 'b'.repeat(40), indexFingerprint: expectedFingerprint }
  ));

  const selected = repo.repositoryFromCommandContext(
    [{ root: '/repo/a' }, { root: '/repo/b' }],
    [{ resourceUri: { fsPath: '/repo/b' } }]
  );
  assert.strictEqual(selected?.root, '/repo/b');
  ...
  assert.deepStrictEqual(await unborn.readProjectRulesAtHead('/repo', '<unborn>'), {
    rules: {}, source: 'unborn-default', fingerprint: '<none>'
  });

  const invalidPolicy = createGitRepository({
    runProcess: async (_command, args) => {
      if (args[0] === 'ls-tree') return { stdout: `100644 blob deadbeef\t.codex-commit.json\0`, stderr: '' };
      if (args[0] === 'show') return { stdout: '{"codexPath":"evil"}', stderr: '' };
  ...
  await assert.rejects(
    () => invalidPolicy.readProjectRulesAtHead('/repo', head),
    /unsupported fields/i
  );

  assert(calls.some(call => call.command === 'git' && call.options.timeoutMs === 15000));
  console.log('git repository snapshot tests passed.');
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});

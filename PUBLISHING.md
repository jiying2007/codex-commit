...
Codex Commit Safe releases are immutable GitHub Actions builds from a committed source revision, locked npm graph, and one exact commit-pinned **Codex Safe Core v4** submodule.
...
- Safe Core v4 / Safe Contract v2 / Policy Schema v3 / Commit Receipt v4 / Prompt Contract v1 checks pass;
- unit, regression, provenance and module-boundary checks pass;
...
The official VSIX runtime is built into `dist/`; development source, tests, scripts, git metadata, lockfile, and build-only configuration must not enter the VSIX. CI inspects the actual packaged VSIX before publication.
...
3. verify SHA256;
4. run `gh attestation verify <vsix> -R jiying2007/codex-commit`;

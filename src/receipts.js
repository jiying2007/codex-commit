...
function createCommitReceiptStore(globalState, { git, normalizeFsPath, fingerprintDiff }) {
  ...
  function restore() {
    ...
    for (const [repoKey, receipts] of Object.entries(stored)) {
      ...
      const valid = receipts.map(validateCommitReceipt).filter(receipt => receipt && (receipt.commitOid === undefined || receipt.commitOid === '<pending>')).slice(0, MAX_RECEIPTS_PER_REPO);
  ...
  async function persistPending(repoRoot, receipt) {
    ...
    ) === index).slice(0, MAX_RECEIPTS_PER_REPO);
  ...
  function getReceipts(repoRoot) { return (receiptsByRepo.get(normalizeFsPath(repoRoot)) || []).map(receipt => ({ ...receipt })); }
  async function getEvidenceForRange(repoRoot, baseRef, headRef = 'HEAD', token) {
    ...
    for (const commitOid of commits) {
      ...
      const diffFingerprint = await fingerprintDiff(diff), messageFingerprint = fingerprintCommitMessage(message);
      const pending = candidates.find(receipt => receipt.diffFingerprint === diffFingerprint && receipt.messageFingerprint === messageFingerprint);
      if (!pending) continue;
      const resolved = validateCommitReceipt({ ...pending, commitOid });
      if (resolved) matches.push({ commitOid, receipt: resolved });
    }
    return Object.freeze({schemaVersion:COMMIT_RECEIPT_SCHEMA_VERSION,kind:'codex-commit-range-evidence',totalCommits:commits.length,generatedCommits:matches.length,reviewedGeneratedCommits:matches.filter(item=>item.receipt.reviewReceiptFingerprint!=='<none>').length,matches:matches.map(item=>({commitOid:item.commitOid,receipt:{...item.receipt}}))});
  }
  async function clear() { receiptsByRepo.clear(); if (globalState) await globalState.update(RECEIPT_STORAGE_KEY, undefined); }
  return Object.freeze({ restore, persistPending, getReceipts, getEvidenceForRange, clear });
}

module.exports = { RECEIPT_STORAGE_KEY, MAX_RECEIPTS_PER_REPO, normalizeCommitMessage, fingerprintCommitMessage, createCommitReceiptStore };

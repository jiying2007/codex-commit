...
function verify() {
  ...
  if (lock.source.repository !== 'jiying2007/codex-safe-core' || lock.source.path !== 'src') fail('Safe Core canonical source must be jiying2007/codex-safe-core/src.');
  ...
  for (const name of names) {
    ...
    if (sha256(fs.readFileSync(path.join(coreDir, name))) !== expected) fail(`Vendored Safe Core file hash mismatch: ${name}.`);
  }
...
function getText(url) {
  return new Promise((resolve, reject) => {
    const request = https.get(url, { headers: { 'User-Agent': 'codex-safe-core-sync', Accept: 'application/octet-stream' } }, response => {
      if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) { response.resume(); return getText(response.headers.location).then(resolve, reject); }
      if (response.statusCode !== 200) { response.resume(); return reject(new Error(`Safe Core download failed (${response.statusCode}): ${url}`)); }
      const chunks = []; response.on('data', chunk => chunks.push(Buffer.from(chunk))); response.on('end', () => resolve(Buffer.concat(chunks)));
    });
...
async function verifyUpstream() {
  const lock = readJson(lockPath); const upstream = await fetchUpstream();
  if (sha256(upstream.manifestBytes) !== lock.manifestSha256) fail('Locked Safe Core manifest differs from canonical upstream; run `node scripts/safe-core.js sync` and review the diff.');
  for (const [name, expected] of Object.entries(lock.files || {})) if (upstream.manifest.files?.[name] !== expected) fail(`Locked upstream hash drifted: ${name}.`);
  verify(); console.log('Safe Core upstream lock verified.');
}
async function sync() {
  const upstream = await fetchUpstream(); fs.mkdirSync(coreDir, { recursive: true }); fs.writeFileSync(path.join(coreDir, 'manifest.json'), upstream.manifestBytes);
  for (const [name, bytes] of Object.entries(upstream.files)) fs.writeFileSync(path.join(coreDir, name), bytes);
  const nextLock = { schemaVersion: 1, safeCoreVersion: upstream.manifest.safeCoreVersion, source: upstream.manifest.source, manifestSha256: sha256(upstream.manifestBytes), files: upstream.manifest.files };
  fs.writeFileSync(lockPath, `${JSON.stringify(nextLock, null, 2)}\n`); verify(); console.log('Safe Core synchronized from canonical upstream; review and commit the resulting diff.');
}
async function main() {
...
if (require.main === module) main().catch(error => { console.error(`Safe Core check failed: ${error.message}`); process.exit(1); });

...
# Adapt the one-time migration to the already-hardened 1.2.4 documentation wording.
...
old_security = "security_anchor = 'Project rules cannot configure the Codex executable, model, environment variables, working directory, or arbitrary commands.\\n'"
new_security = "security_anchor = 'Project configuration cannot set the Codex executable, model, environment variables, working directory, or arbitrary commands. The executable path and model are application-scoped User Settings and are checked again at runtime.\\n'"
if text.count(old_en) != 1 or text.count(old_zh) != 1 or text.count(old_security) != 1:
    raise SystemExit('Scope Intelligence migration documentation anchors changed unexpectedly')
text = text.replace(old_en, new_en, 1).replace(old_zh, new_zh, 1).replace(old_security, new_security, 1)
